...
class Scan_msg:
    ...
    def move_goal(self, x, y,vel,isForward):
	r=rospy.Rate(1)
	rospy.loginfo("Move goal initiated")	
	print 'Move to goal'
	constraint = abs(self.x-x)>1# and self.y!=y	
	print constraint       
	
	
	
	while abs(self.x-x)>0.6 and abs(self.y-y)>0.6:
